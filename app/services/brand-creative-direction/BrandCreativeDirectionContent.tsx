...
export default function BrandCreativeDirectionContent({
  brandCases,
  stylingCases,
}: {
  brandCases: BrandCreativeCase[];
  stylingCases: BrandCreativeCase[];
}) {
    ...
    <main className="min-h-screen bg-cream text-red">
      ...
      {brandCases.length > 0 && (
        <CaseCarousel
          ...
          description="Creative direction for products, campaigns, and brand experiences that connect with people."
          items={brandCases}
          sliderRef={brandSliderRef}
        />
      )}

      {stylingCases.length > 0 && (
        <CaseCarousel
          id="styling-portfolio"
          title="Fashion Styling"
          description="Product styling for editorials, campaigns, e-commerce, and social media."
          items={stylingCases}
